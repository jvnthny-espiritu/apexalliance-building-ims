...
export default function FilterDate() {
  ...
  useEffect(() => {
    // Fetch date names from the database when the component mounts
    async function fetchDates() {
      try {
        const fetchedDates = await fetchDateNames();
              ...
              <Menu.Item key={index}>
                {({ active }) => (
                  <a
                    ...
                    onClick={() => setSelectedDate(dateName)}
                  >
                    {dateName}
                  </a>
                )}
