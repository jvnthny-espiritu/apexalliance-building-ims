...
const fetchTypeAssetsNames = async () => {
  return [
    "Furniture",
    "Safety Materials"
  ];
...
export default function FilterTypeAssets() {
  const [selectedTypeAssets, setSelectedTypeAssets] = useState(null);
  const [typeassetss, setTypeAssetss] = useState([]);

  useEffect(() => {
    async function fetchTypeAssetss() {
      try {
        const fetchedTypeAssetss = await fetchTypeAssetsNames();
        setTypeAssetss(fetchedTypeAssetss);
      } catch (error) {
        console.error('Error fetching typeassetss:', error);
      }
    }
    ...
    <Menu as="div" className="relative inline-block text-left">
      ...
      >
        <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-70 focus:outline-none">
          <div className="py-1">
            {typeassetss.map((typeassetsName, index) => (
              <Menu.Item key={index}>
                {({ active }) => (
                  <a
                    href="#"
                    ...
                    onClick={() => setSelectedTypeAssets(typeassetsName)}
                  >
                    {typeassetsName}
                  </a>
                )}
